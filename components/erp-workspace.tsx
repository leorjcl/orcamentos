...
import { QuotesOrders } from "./quotes-orders";
...
type View = "dashboard" | "pricing" | "products" | "settings" | "receipts" | "orders" | "quotes";
...
const nav = [
  { view: "quotes", href: "/orcamentos", label: "Orçamentos e pedidos", icon: "file" },
  ...
  { view: "orders", href: "/ordens-servico", label: "Ordens de serviço", icon: "printer" },
  { view: "receipts", href: "/recibos", label: "Recibos", icon: "file" },
];
const future = [ ["people", "Clientes"], ["printer", "Produção"], ["box", "Controle de estoque"], ["chart", "Relatórios e custos"]];
...
export function ErpWorkspace({ view }: { view: View }) {
  ...
  const titles = { dashboard: "Visão geral", pricing: "Precificação", products: "Produtos e serviços", settings: "Configurações", receipts: "Recibos", orders: "Ordens de serviço", quotes: "Orçamentos e pedidos" };
  ...
  return <div className="erp-shell">
    ...
    <div className="workspace">
      ...
      <main id="main-content" className="main-content">
        {view !== "orders" && view !== "quotes" && <div className="local-banner"><Icon name="file" size={17} /><span><strong>Prévia do novo ERP.</strong> Rascunhos ficam apenas neste navegador. Precificações ainda não são sincronizadas com o WordPress.</span></div>}
        ...
        {view === "quotes" && <QuotesOrders />}
        {view === "orders" && <section aria-label="Ordens de serviço">
          <div className="page-heading"><div><h1>Ordens de serviço</h1><p>Produção, pagamentos e relatórios da gráfica.</p></div><a className="secondary" href="/ordens-servico/index.html" target="_blank" rel="noopener noreferrer">Abrir em tela cheia</a></div>
          <iframe title="Ordens de serviço YGPrint" src="/ordens-servico/index.html" style={{ width: "100%", height: "max(900px, calc(100dvh - 160px))", border: "1px solid #e2e8f0", borderRadius: 12, background: "#f8fafc" }} />
        </section>}
